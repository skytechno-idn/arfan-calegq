...
const PartaiCard = ({ src, color, name, number, onClick,status=true }: any) => {
  return (
    <>
      <button
      ...
      >
        <div className="absolute right-0 -top-1 z-10">
            {status ? (
                <Chip color="success" size="sm" > <CheckCircle className="w-5 h-4  text-white"/></Chip>
            ):(
<Chip color="warning"   size="sm">  <Spinner color="default" className="w-3 py-1" size="sm" /></Chip>
            )}
           
        </div>
        <div className="group-hover:absolute inset-0 z-20 bg-white/50"></div>
